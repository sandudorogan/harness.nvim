...
local out_path = assert(os.getenv("HARNESS_ATTACH_OUT"), "HARNESS_ATTACH_OUT")
...
send_obj({ id = "l1", method = "session.list", params = {} })
local l1 = expect_reply("l1")
assert(l1.ok, vim.json.encode(l1))
local sessions = l1.result.sessions
assert(type(sessions) == "table" and #sessions >= 1, "session.list empty")

local sid = sessions[1].id
assert(type(sid) == "string", "session id")

send_obj({ id = "r1", method = "session.resume", params = { sessionId = sid } })
local r1 = expect_reply("r1")
assert(r1.ok, vim.json.encode(r1))
assert(r1.result.alreadyLoaded == true, "expected alreadyLoaded when first client holds session")
